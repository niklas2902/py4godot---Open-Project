...
class CharHandler(CharacterBody3D, Draw):
    ...
    def apply_root_motion(self, delta: float, angle: float):
        ...
        Transform3D.new2(Basis.new3(self.animation_tree.get_root_motion_scale(),
                                    self.animation_tree.get_root_motion_rotation()),
                         self.animation_tree.get_root_motion_position())
        self.root_motion = Transform3D.new2(Basis.new3(self.animation_tree.get_root_motion_scale(),
                                                       self.animation_tree.get_root_motion_rotation()),
                                            self.animation_tree.get_root_motion_position())
        self.orientation *= self.root_motion

        h_velocity: Vector3 = self.orientation.origin / delta
        self.velocity.set_axis(0, h_velocity.x)
        self.velocity.set_axis(2, h_velocity.z)
        # Fix -1 in movement
        self.set_velocity(self.velocity * -1 * delta)
        self.set_up_direction(Vector3.new3(0, 1, 0))
